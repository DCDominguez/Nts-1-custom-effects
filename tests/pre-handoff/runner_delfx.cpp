...
struct Stats{double abs=0,abs_late=0;float peak=0;uint64_t frames=0;};
...
int main(int argc,char**argv){
 ...
 for(float t:vals)for(float d:vals){DELFX_INIT(0,0);DELFX_RESUME();settle(t,d,1);Stats s;std::fill(b,b+B*2,0.0f);b[0]=b[1]=.7f;process(b,B,s);double early=0,late=0;const uint32_t total=20*48000/B;
   for(uint32_t block=0;block<total;++block){std::fill(b,b+B*2,0.0f);process(b,B,s);double e=0;for(float x:b)e+=std::fabs(x);if(block<5*48000/B)early+=e;if(block>18*48000/B)late+=e;}
   if(early>1.0e-3)heard=true; if(early>bestEarly){bestEarly=early;bestLate=late;}
 }
 check(heard,"no delayed/tail response detected from impulse across parameter grid");
 if(finite) check(bestLate < std::max(1.0e-5,bestEarly*0.02),"finite delay does not return near rest");
 else check(bestLate < std::max(1.0e-4,bestEarly*1.2),"tail energy grows instead of remaining bounded");
 std::printf("PASS impulse delivery/tail bound; early %.6f late %.6f\n",bestEarly,bestLate);
