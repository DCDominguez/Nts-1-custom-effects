...
static float phase_advance(float rate){MODFX_INIT(0,0);settle(rate,.9f,300);float start=ph;std::vector<float>z(12000*2),o(12000*2),s(12000*2),sy(12000*2);MODFX_PROCESS(z.data(),o.data(),s.data(),sy.data(),12000);float d=ph-start;if(d<0)d+=1.f;return d;}
int main(){
 ...
 // RATE contract is verified at the actual production modulation phase; audible processing is independently proven above.
 const float slowAdv=phase_advance(.05f), fastAdv=phase_advance(.95f);req(fastAdv>slowAdv*5.f,"RATE does not materially increase modulation phase advance");
 MODFX_INIT(0,0);settle(.95f,.9f);auto fast=run(240000,431);auto fl=hs_measure::channel(fast,0),fr=hs_measure::channel(fast,1);req(hs_measure::stats(fl).peak<=1.001&&hs_measure::stats(fr).peak<=1.001,"output exceeded bounds");
