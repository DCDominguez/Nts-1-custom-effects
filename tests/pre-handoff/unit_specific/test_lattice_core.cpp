...
static void process_block(const std::vector<float>& in,std::vector<float>& out){std::vector<float> sub(in.size(),0.0f),sy(in.size(),0.0f);MODFX_PROCESS(in.data(),out.data(),sub.data(),sy.data(),static_cast<uint32_t>(in.size()/2));}
...
int main(){
  ...
  // Voice-population contract: current 0.3-0 design is bounded 0..16 and rises with TIME.
  require(voice_count(0.04f)>=1,"TIME just above disengage did not enable first voice"); require(voice_count(0.50f)>voice_count(0.20f),"TIME does not increase voice population"); require(voice_count(1.0f)<=16,"voice population exceeded implementation ceiling");
  ...
  // High TIME remains finite/bounded in host execution. This does NOT certify MkI callback/CPU margin.
  ...
  // Maximum TIME must engage recent-history freeze, stop history writes, and keep producing captured audio when source changes.
  MODFX_INIT(0,0); set_controls(0.0f,0.5f); run_frames(90000,0.45f,211.0); const uint32_t before=s_write; set_controls(1.0f,0.5f); run_frames(24000,0.45f,211.0); require(s_freeze_active,"maximum TIME did not engage freeze"); const uint32_t frozen_write=s_write; require(frozen_write>=before||before-frozen_write<kBufferSize,"invalid write index after freeze"); std::vector<float> frozen; run_frames(96000,0.0f,211.0,&frozen); require(s_write==frozen_write,"history continued writing while freeze active"); auto fl=hs_measure::stats(hs_measure::channel(frozen,0),12000); require(fl.rms>0.01,"freeze failed to replay captured history into silence");

  // Freeze release must resume history acquisition.
  ...
  std::puts("LATTICE CORE A-class project-specific harness PASS (hardware high-TIME runtime issue remains open)"); return 0;
}
