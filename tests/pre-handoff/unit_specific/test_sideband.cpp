...
static std::pair<double,double> shifted_freqs(float time,double src){MODFX_INIT(0,0);settle(time,1.f,260);auto y=run(160000,src);auto l=hs_measure::channel(y,0),r=hs_measure::channel(y,1);double fl=hs_measure::zero_cross_frequency(l,48000,12000,120000),fr=hs_measure::zero_cross_frequency(r,48000,12000,120000);return{fl,fr};}
int main(){
 ...
 // Full divergence must translate the two channels to opposite sides of the source frequency. Which physical side is 'up' depends on Hilbert sign convention.
 const double src=1200.0;auto f=shifted_freqs(.22f,src);req(f.first>0&&f.second>0,"full divergence lost translated tone");req((f.first-src)*(f.second-src)<0.0,"full divergence does not place channels on opposite sidebands");req(std::fabs(f.first-src)>40.0&&std::fabs(f.second-src)>40.0,"sideband translation is too small at tested setting");
 // TIME/SHIFT must increase total L/R translation separation.
 auto low=shifted_freqs(.10f,1000.0),high=shifted_freqs(.35f,1000.0);double lowSep=std::fabs(low.first-low.second),highSep=std::fabs(high.first-high.second);req(highSep>lowSep+250.0,"SHIFT/TIME does not materially increase sideband separation");
 MODFX_INIT(0,0);settle(.22f,1.f);auto shifted=run(120000,1200);auto l=hs_measure::channel(shifted,0),r=hs_measure::channel(shifted,1);req(hs_measure::stats(l).peak<=1.001&&hs_measure::stats(r).peak<=1.001,"shift output exceeded bounds");req(hs_measure::max_abs_diff(l,r)>0.01,"DIVERGENCE does not create stereo translation difference");
