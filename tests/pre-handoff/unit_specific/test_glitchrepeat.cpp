...
static void set(float sl,float ch,float mix){DELFX_PARAM(k_user_delfx_param_time,q(sl));DELFX_PARAM(k_user_delfx_param_depth,q(ch));DELFX_PARAM(k_user_delfx_param_shift_depth,q(mix));}
static void settle(float a,float b,float m,int n=260){set(a,b,m);std::vector<float>z(128);for(int i=0;i<n;i++)DELFX_PROCESS(z.data(),64);}
...
static std::vector<float>signal(std::size_t n){std::vector<float>x(n*2);for(std::size_t i=0;i<n;i++){float v=.25f*std::sin(2*hs_measure::kPi*(220.0+((i/1000)%5)*73.0)*i/48000.0)+.000002f*(float)(i%10000);x[2*i]=v;x[2*i+1]=.9f*v;}return x;}
int main(){
 ...
 // BPM controls the first arm boundary. Do not pre-roll here because pre-roll itself advances beatCnt.
 DELFX_INIT(0,0);test_bpm=120;set(.2f,1,1);auto pre=signal(14000);proc(pre);bool active120=repeats>0;
 DELFX_INIT(0,0);test_bpm=240;set(.2f,1,1);auto fast=signal(14000);proc(fast);bool active240=repeats>0;req(!active120&&active240,"tempo does not move repeat-arm boundary");
 // Low BPM fallback is 120 BPM and therefore must also remain pre-arm at 14k samples.
 DELFX_INIT(0,0);test_bpm=0;set(.2f,1,1);auto fb=signal(14000);proc(fb);req(repeats==0,"BPM fallback did not behave like 120 BPM before first beat");
 // Fresh init must restore the default deterministic state, including control smoothing and RNG.
 DELFX_INIT(0,0);test_bpm=120;set(.4f,1,1);auto a=signal(50000);proc(a);DELFX_INIT(0,0);test_bpm=120;set(.4f,1,1);auto b=signal(50000);proc(b);req(hs_measure::max_abs_diff(a,b)<1e-6,"fresh init does not reproduce repeat trajectory");
