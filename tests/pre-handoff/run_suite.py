...
#include <cstdlib>
#include <vector>
static int32_t q31p(float x){ if(x<0)x=0; if(x>.999999f)x=.999999f; return (int32_t)(x*2147483647.f); }
static bool finite_vec(const std::vector<float>&v){ for(float x:v) if(!std::isfinite(x)) return false; return true; }
static float energy(const std::vector<float>&v){ double e=0; for(float x:v)e+=(double)x*x; return (float)e; }
...
int main(){
  ...
  double delivered=0; for(int k=0;k<12;k++){ fill_sine(in,220.f,.35f,(float)k*B*220.f/48000.f); MODFX_PROCESS(in.data(),out.data(),sub.data(),sout.data(),B); if(!finite_vec(out)||maxabs(out)>8.f) fail("normal signal bound"); delivered+=energy(out); }
  if(delivered<1e-8) fail("normal signal delivery");
  for(int k=0;k<1200;k++){ float a=(k%200)/199.f,b=((k*37)%200)/199.f; MODFX_PARAM(k_user_modfx_param_time,q31p(a)); MODFX_PARAM(k_user_modfx_param_depth,q31p(b)); fill_sine(in,55.f+(k%12)*73.f,.55f,(float)k*.013f); MODFX_PROCESS(in.data(),out.data(),sub.data(),sout.data(),B); if(!finite_vec(out)||maxabs(out)>8.f) fail("sweep/soak bound"); }
...
int main(){
  ...
  DELFX_INIT(0,0); DELFX_PARAM(k_user_delfx_param_time,q31p(.5f)); DELFX_PARAM(k_user_delfx_param_depth,q31p(.95f)); DELFX_PARAM(k_user_delfx_param_shift_depth,q31p(.85f));
  const int sourceBlocks=1000, tailBlocks=6000; double sourceE=0,tailE=0;
  for(int k=0;k<sourceBlocks;k++){ fill_sine(x,110.f+(k%4)*55.f,.38f,(float)k*.017f); DELFX_PROCESS(x.data(),B); if(!finite_vec(x)||maxabs(x)>8.f) fail("source/capture bound"); sourceE+=energy(x); }
  std::fill(x.begin(),x.end(),0.f);
  for(int k=0;k<tailBlocks;k++){ DELFX_PROCESS(x.data(),B); if(!finite_vec(x)||maxabs(x)>8.f) fail("tail bound"); tailE+=energy(x); std::fill(x.begin(),x.end(),0.f); }
  if(sourceE<1e-8) fail("source delivery"); if(tailE<1e-9) fail("no delayed/captured delivery");
  for(int k=0;k<1800;k++){ float a=(k%240)/239.f,b=((k*41)%240)/239.f,m=((k*17)%240)/239.f; DELFX_PARAM(k_user_delfx_param_time,q31p(a)); DELFX_PARAM(k_user_delfx_param_depth,q31p(b)); DELFX_PARAM(k_user_delfx_param_shift_depth,q31p(m)); fill_sine(x,80.f+(k%16)*61.f,.5f,(float)k*.011f); DELFX_PROCESS(x.data(),B); if(!finite_vec(x)||maxabs(x)>8.f) fail("sweep/soak bound"); }
...
def build_one(name,typ,rel):
    ...
    code={"modfx":harness_modfx,"delfx":harness_delfx,"revfx":harness_revfx,"osc":harness_osc}[typ](src)
    with tempfile.TemporaryDirectory(prefix="prehand_") as td:
        cpp=pathlib.Path(td)/"test.cpp"; exe=pathlib.Path(td)/"test"; cpp.write_text(code)
        cp=subprocess.run(["g++","-std=c++17","-O1","-g","-fsanitize=address,undefined","-fno-omit-frame-pointer",f"-I{STUBS}",str(cpp),"-lm","-o",str(exe)],text=True,stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
...
def main():
    ...
    for name,typ,rel in UNITS:
        ok,note=build_one(name,typ,rel); rows.append((name,typ,"PASS" if ok else "FAIL",note)); failed+=0 if ok else 1; print(f"{name}: {'PASS' if ok else 'FAIL'} - {note}")
    OUT.parent.mkdir(parents=True,exist_ok=True)
    with OUT.open("w") as f:
        f.write("# Suite pre-handoff common-gate results\n\n")
        f.write("Deterministic production-DSP common gate from `PRE_HANDOFF_PROTOCOL.md`. This does not certify physical MkI loading, real-time CPU margin, or subjective tone/musicality.\n\n")
