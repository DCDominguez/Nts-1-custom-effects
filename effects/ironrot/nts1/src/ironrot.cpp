...
static inline float asym_sat(float x, float asym) {
  // Bias is removed after saturation so digital silence remains silent.
...
static inline float crossover_damage(float x, float amount) {
  // A bounded crossover/dead-zone stage makes CORROSION perceptually obvious
  // without relying on a level increase. It is strongest for bright CHARACTER.
...
static inline float process_one(float x, ChannelState &st, float ch, float cor) {
  // Stage 1: CHARACTER-selectable spectral pre-emphasis.
  ...
  // Stage 2: asymmetric primary saturation.
  const float drive1 = 1.0f + 17.0f * cor * cor;
  float y = asym_sat(shaped * drive1, 0.24f + 0.66f * ch);

  // Stage 3: crossover fracture plus a separately driven edge path. This is
  // deliberately more audible than M2.0 so CORROSION changes texture, not level.
  ...
  // Stage 4: cleanup / bass retention.
  ...
  // Slow envelope comparison removes the loudness advantage of distortion.
  // Gain is attenuation-only: IRONROT may become slightly quieter at extreme
  // corrosion, but it should not masquerade as "better" simply by getting louder.
  ...
  match = clampf(match, 0.32f, 1.0f);

  const float severity_trim = 1.0f - 0.12f * cor;
